      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", paddingTop: 80 }}>
          ...
          <p style={{ fontSize: 13, color: "#475569", marginTop: 6 }}>
            {search ? "Try a different search term." : "Calls will appear here once Twilio is connected."}
          </p>
        </div>
      ) : (
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ margin: 0, fontWeight: 600, color: "#f1f5f9", fontSize: 14 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {call.transcript.map((msg, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: msg.role === "user" ? "flex-end" : "flex-start" }}>
                          <div style={{
                            maxWidth: "75%", padding: "8px 14px", borderRadius: 10, fontSize: 13,
                            background: msg.role === "user" ? "rgba(99,102,241,0.15)" : "#0f1117",
                            color: msg.role === "user" ? "#a5b4fc" : "#cbd5e1",
                            border: "1px solid " + (msg.role === "user" ? "#6366f133" : "#2d3748"),
                          }}>
                            <span style={{ fontWeight: 600, fontSize: 11, opacity: 0.6, textTransform: "uppercase", display: "block", marginBottom: 2 }}>
                              {msg.role === "user" ? "Caller" : "Alex"}
                            </span>
                            {msg.content}
                          </div>
                        </div>
